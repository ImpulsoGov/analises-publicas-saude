...
def main(row, place_id, config, place_specific_params, symptomatic=0.4):
    """
    row: pd.Series
    ...
    symtomatic = [int(row["active_cases"] * symptomatic) if not np.isnan(row["active_cases"]) else 1][0]
    ...
    params = {
        ...
        "place_specific_params": {
            "fatality_ratio": place_specific_params["fatality_ratio"].loc[row.name],
            "i1_percentage": place_specific_params["i1_percentage"].loc[row.name],
            "i2_percentage": place_specific_params["i2_percentage"].loc[row.name],
            "i3_percentage": place_specific_params["i3_percentage"].loc[row.name],
        },
