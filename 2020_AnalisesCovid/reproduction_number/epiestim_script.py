...
def run_epiestim(group):
    ...
    # Filtra > 15 dias para calculo
    if len(group) < 15:
        return
    # Filtra series nao negativas
    if any(group["I"] < 0):
        return

    # Converte para serie em R
    ...
    # Recupera coluna de datas - comeca depois de 7 dias
    rt = (
...
def get_rt(df, place_id):
    ...
    # Agrega e filtra a serie de casos: > 100 casos confirmados
    df_cases = (
...
def now(config=None):
    # Para testar:
    return get_rt(pd.read_csv("http://datasource.coronacidades.org/br/health_region/cases/full"), 
