...
def get_cases_series(df, place_type, min_days):
    ...
    df = (
        df[~df["notification_rate"].isnull()][
            [place_type, "last_updated", "daily_cases"]
        ]
        .groupby([place_type, "last_updated"])["daily_cases"]
        .sum()
        .round(0)
    )
...
def now(config=None):
    ...
    df = pd.read_csv("http://datasource.coronacidades.org/br/health_region/cases/full")
    df["last_updated"] = pd.to_datetime(df["last_updated"])
    
    place_type = "health_region_id"

    # Filter more than 14 days
    df = get_cases_series(df, place_type, config["br"]["rt_parameters"]["min_days"])

    # subs cidades com 0 casos -> 0.1 caso no periodo
    df = df.replace(0, 0.1)

    # Run in parallel
    return sequential_run(df, config, place_type)
